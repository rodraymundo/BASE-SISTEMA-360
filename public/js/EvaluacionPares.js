...
const listaPares = document.getElementById('listaPares'); // DIV QUE ALMACENARA LAS CARD DE PARES 
...
async function cargarPares() {
    try {
        const res = await fetch('/getPares', { credentials: 'include' }); //OBTENER LOS PARES CON LA API
        const data = await res.json();
        if (!data.success) throw new Error('No se pudieron obtener los pares');

        data.pares.forEach(par => {
            let estadoEvaluacion = '';
            switch (par.estado_evaluacion_par) { //SEFUN EL ESTADO DE EVALUACION SE PONE UNA OPCION DIFERENTE 
                case 0:
                        estadoEvaluacion = `<button class="btn btn-success rounded-pill px-4 shadow evaluar-btn" id="btnEvaluar-${par.id_personal}" data-id_personal="${par.id_personal}">Evaluar</button>`;
                    break;
                case 1:
                        estadoEvaluacion = `<p class="fw-bold text-uppercase text-success">Par evaluado correctamente</p>`;
                    break;            
                default:
                    break;
            }

            const nombrePar = `${par.nombre_personal} ${par.apaterno_personal} ${par.amaterno_personal}`;

            const cardPar = document.createElement('div'); // SE CREA UN ELEMENTO HTML QUE ALMACENARA EL CONTENIDO
            cardPar.className = 'col mb-3'; // SE LE AGREGAN LAS CLASES PRINCIPALES A EL ELEMENTO HTML Y ABAJO EL CONTENIDO QUE ALMACENARA
            cardPar.innerHTML = `  
                <div class="card h-100 shadow rounded-4 p-4">
                    <img src='./assets/img/${par.img_personal}' class="card-img-top rounded-top-4" alt="Imagen del Par">
                    <div class="card-body text-center">
                        <h5 class="fw-bold text-uppercase bg-danger text-white rounded-pill py-2 px-3 d-inline-block shadow-lg">${nombrePar}</h5>
                        <p class="card-text mb-1">${par.nombre_puesto}</p>
                    </div>
            ...
            `; 
            listaPares.appendChild(cardPar); // SE EL AGREGA EL ELEMENTO CREADO A EL ELEMENTO HTML ORIGINAL EL EL .html
        });

        const cards = listaPares.querySelectorAll('.col');
        ...
        console.error('Error al cargar pares:', error);
    }
}
...
async function cargarPreguntasModal(id_personal) {
    try {
        const res = await fetch(`/getPreguntasPar`, { credentials: 'include' }); //OBTENER LAS PREGUNTAS DE LA API
        const data = await res.json();
        ...
        // HEADER - MODAL
        const tituloModal = 'PAR';// AGREGAR TITULO 
        ...
        if (totalPreguntas % 3 == 0 || totalPreguntas % 3 == 2){ // ES 0 PORQUE ESO QUIERE DECIR QUE EL TOTAL DE PREGUNTAS ES MULTIPLO DE 3 PUES NO HAY RESIDUO  Y QUE LA ULTIMA PAGINA ESTA OCUPADA POR 3 PREGUNTAS
            ...
            paginaModalBody.innerHTML = `
                ...
                <textarea class="form-control mb-3 d-none" placeholder="Aspectos a positivos en ${tituloModal.toLowerCase()}: ..." id="comentarioParPositivo"></textarea>
                <div class="mb-3 p-3 rounded-4" style="background-color: #eF2d3b">
                ...
                <textarea class="form-control mb-3 d-none" placeholder="Aspectos a negativos en ${tituloModal.toLowerCase()}: ..." id="comentarioParNegativo"></textarea>
            ...
            contenidoComentarioPositivo.id = 'comentarioParPositivo';
            ...
            contenidoComentarioNegativo.id = 'comentarioParNegativo';
        ...
        respuestasComentarioPositivo.forEach(input => { // HACER QUE CADA OPCION (INPUT RADIO) TENGA UN EVENTO PARA SABER CUANDO UNO ESTA SELECCIONADO
            input.addEventListener('change', () => {
                ...
                const comentarioParPositivo = document.getElementById('comentarioParPositivo');
                // DEPENDIENDO DE LA RESPUESTA APARECE EL ESPPACIO PARA COMENTAR O NO
                if (comentarioPositivoSeleccionValor === "1") {
                    comentarioParPositivo.classList.remove('d-none');
                }else{
                    comentarioParPositivo.classList.add('d-none');
                }
            });
        });
        ...
        respuestasComentarioNegativo.forEach(input => { // HACER QUE CADA OPCION (INPUT RADIO) TENGA UN EVENTO PARA SABER CUANDO UNO ESTA SELECCIONADO
            input.addEventListener('change', () => {
                ...
                const comentarioParNegativo = document.getElementById('comentarioParNegativo');
                // DEPENDIENDO DE LA RESPUESTA APARECE EL ESPPACIO PARA COMENTAR O NO
                if (comentarioNegativoSeleccionValor === "1") {
                    comentarioParNegativo.classList.remove('d-none');
Par
                    comentarioParNegativo.classList.add('d-none');
                }
            });
        });
        ...
        btnTerminarEvaluacion.addEventListener('click', async ()=>{
            ...
            if (comentarioPositivoSeleccionValor == 1 && comentarioParPositivo.value == ''){ // QUIERE DECIR QUE SI TIENE COMENTARIO POSITIVO PERO ESTA VACIO POR LO QUE SE ALERTA 
                await Swal.fire({
                    icon: 'error',
            ...
            if (comentarioNegativoSeleccionValor == 1 && comentarioParNegativo.value == ''){ // QUIERE DECIR QUE SI TIENE COMENTARIO NEGATIVO PERO ESTA VACIO POR LO QUE SE ALERTA 
                await Swal.fire({
                    icon: 'error',
                    title: 'El apartado para comentar esta vacio'
                });
            ...
            if (comentarioPositivoSeleccionValor == 1 && comentarioParPositivo.value != ''){ // QUIERE DECIR QUE SI TIENE COMENTARIO POSITIVO Y QUE NO ESTA VACIO EL COMENTARIO
                comentariosPersonal.push({
                    'tipo_comentario': 1,
                    'comentario_personal': comentarioParPositivo.value 
                });
            }

            if (comentarioNegativoSeleccionValor == 1 && comentarioParNegativo.value != ''){ // QUIERE DECIR QUE SI TIENE COMENTARIO NEGATIVO Y QUE NO ESTA VACIO EL COMENTARIO
                comentariosPersonal.push({
                    'tipo_comentario': 0,
                    'comentario_personal': comentarioParNegativo.value 
                });
            }
            ...
            const resulado = await guardarRespuestasPar(id_personal,respuestasPersonal,comentariosPersonal);
            if(resulado.success){
                await Swal.fire({
                    icon: 'success',
...
async function guardarRespuestasPar (id_personal,respuestas,comentarios){
    try {
        ...
        const res = await fetch('/postRespuestasPar', {
            method: 'POST',
            headers: {
