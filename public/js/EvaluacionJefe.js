...
const listaJefes = document.getElementById('listaJefes'); // DIV QUE ALMACENARA LAS CARD DE JEFES 
...
async function cargarJefes() {
    try {
        const res = await fetch('/getJefes', { credentials: 'include' }); //OBTENER LOS JEFE CON LA API
        const data = await res.json();
        if (!data.success) throw new Error('No se pudieron obtener los jefes');

        data.jefes.forEach(jefe => {
            let estadoEvaluacion = '';
            switch (jefe.estado_evaluacion_jefe) { //SEFUN EL ESTADO DE EVALUACION SE PONE UNA OPCION DIFERENTE 
                case 0:
                        estadoEvaluacion = `<button class="btn btn-success rounded-pill px-4 shadow evaluar-btn" id="btnEvaluar-${jefe.id_personal}" data-id_personal="${jefe.id_personal}">Evaluar</button>`;
                    break;
                case 1:
                        estadoEvaluacion = `<p class="fw-bold text-uppercase text-success">Personal evaluado correctamente</p>`;
                    break;            
                default:
                    break;
            }

            const nombreJefe = `${jefe.nombre_personal} ${jefe.apaterno_personal} ${jefe.amaterno_personal}`;

            const cardJefe = document.createElement('div'); // SE CREA UN ELEMENTO HTML QUE ALMACENARA EL CONTENIDO
            cardJefe.className = 'col mb-3'; // SE LE AGREGAN LAS CLASES PRINCIPALES A EL ELEMENTO HTML Y ABAJO EL CONTENIDO QUE ALMACENARA
            cardJefe.innerHTML = `  
                <div class="card h-100 shadow rounded-4 p-4">
                    <img src='./assets/img/${jefe.img_personal}' class="card-img-top rounded-top-4" alt="Imagen del Jefe">
                    <div class="card-body text-center">
                        <h5 class="fw-bold text-uppercase bg-danger text-white rounded-pill py-2 px-3 d-inline-block shadow-lg">${nombreJefe}</h5>
                        <p class="card-text mb-1">${jefe.nombre_puesto}</p>
                    </div>
            ...
            `; 
            listaJefes.appendChild(cardJefe); // SE EL AGREGA EL ELEMENTO CREADO A EL ELEMENTO HTML ORIGINAL EL EL .html
            
        });
        

        const cards = listaJefes.querySelectorAll('.col');
...
async function cargarPreguntasModal(id_personal) {
    try {
        const res = await fetch(`/getPreguntasJefe`, { credentials: 'include' }); //OBTENER LAS PREGUNTAS DE LA API
        const data = await res.json();
        ...
        // HEADER - MODAL
        const tituloModal = 'JEFE';// AGREGAR TITULO 
        ...
        if (totalPreguntas % 3 == 0 || totalPreguntas % 3 == 2){ // ES 0 PORQUE ESO QUIERE DECIR QUE EL TOTAL DE PREGUNTAS ES MULTIPLO DE 3 PUES NO HAY RESIDUO  Y QUE LA ULTIMA PAGINA ESTA OCUPADA POR 3 PREGUNTAS
            ...
            paginaModalBody.innerHTML = `
                ...
                <textarea class="form-control mb-3 d-none" placeholder="Aspectos a positivos en ${tituloModal.toLowerCase()}: ..." id="comentarioJefePositivo"></textarea>
                <div class="mb-3 p-3 rounded-4" style="background-color: #eF2d3b">
                ...
                <textarea class="form-control mb-3 d-none" placeholder="Aspectos a negativos en ${tituloModal.toLowerCase()}: ..." id="comentarioJefeNegativo"></textarea>
            ...
            contenidoComentarioPositivo.id = 'comentarioJefePositivo';
            ...
            contenidoComentarioNegativo.id = 'comentarioJefeNegativo';
        ...
        respuestasComentarioPositivo.forEach(input => { // HACER QUE CADA OPCION (INPUT RADIO) TENGA UN EVENTO PARA SABER CUANDO UNO ESTA SELECCIONADO
            input.addEventListener('change', () => {
                ...
                const comentarioJefePositivo = document.getElementById('comentarioJefePositivo');
                // DEPENDIENDO DE LA RESPUESTA APARECE EL ESPPACIo PARA COMENTAR O NO
                if (comentarioPositivoSeleccionValor === "1") {
                    comentarioJefePositivo.classList.remove('d-none');
                }else{
                    comentarioJefePositivo.classList.add('d-none');
                }
            });
        });
        ...
        respuestasComentarioNegativo.forEach(input => { // HACER QUE CADA OPCION (INPUT RADIO) TENGA UN EVENTO PARA SABER CUANDO UNO ESTA SELECCIONADO
            input.addEventListener('change', () => {
                ...
                const comentarioJefeNegativo = document.getElementById('comentarioJefeNegativo');
                // DEPENDIENDO DE LA RESPUESTA APARECE EL ESPPACIO PARA COMENTAR O NO
                if (comentarioNegativoSeleccionValor === "1") {
                    comentarioJefeNegativo.classList.remove('d-none');
Par
                    comentarioJefeNegativo.classList.add('d-none');
                }
            });
        });
        ...
        btnTerminarEvaluacion.addEventListener('click', async ()=>{
            ...
            if (comentarioPositivoSeleccionValor == 1 && comentarioJefePositivo.value == ''){ // QUIERE DECIR QUE SI TIENE COMENTARIO POSITIVO PERO ESTA VACIO POR LO QUE SE ALERTA 
                await Swal.fire({
                    icon: 'error',
            ...
            if (comentarioNegativoSeleccionValor == 1 && comentarioJefeNegativo.value == ''){ // QUIERE DECIR QUE SI TIENE COMENTARIO NEGATIVO PERO ESTA VACIO POR LO QUE SE ALERTA 
                await Swal.fire({
                    icon: 'error',
                    title: 'El apartado para comentar esta vacio'
                });
            ...
            if (comentarioPositivoSeleccionValor == 1 && comentarioJefePositivo.value != ''){ // QUIERE DECIR QUE SI TIENE COMENTARIO POSITIVO Y QUE NO ESTA VACIO EL COMENTARIO
                comentariosPersonal.push({
                    'tipo_comentario': 1,
                    'comentario_personal': comentarioJefePositivo.value 
                });
            }

            if (comentarioNegativoSeleccionValor == 1 && comentarioJefeNegativo.value != ''){ // QUIERE DECIR QUE SI TIENE COMENTARIO NEGATIVO Y QUE NO ESTA VACIO EL COMENTARIO
                comentariosPersonal.push({
                    'tipo_comentario': 0,
                    'comentario_personal': comentarioJefeNegativo.value 
                });
            }
            ...
            const resulado = await guardarRespuestasJefe(id_personal,respuestasPersonal,comentariosPersonal);
            if(resulado.success){
                await Swal.fire({
                    icon: 'success',
...
async function guardarRespuestasJefe (id_personal,respuestas,comentarios){
    try {
        ...
        const res = await fetch('/postRespuestasJefe', {
            method: 'POST',
            headers: {
