...
const lista360 = document.getElementById('lista360'); // DIV QUE ALMACENARA LAS CARD DE 360 
...
async function cargar360() {
    try {
        const res = await fetch('/get360', { credentials: 'include' }); //OBTENER LOS 360 CON LA API
        const data = await res.json();
        if (!data.success) throw new Error('No se pudieron obtener los 360');

        data.todos.forEach(personal => {
            let estadoEvaluacion = '';
            switch (personal.estado_evaluacion_360) { //SEFUN EL ESTADO DE EVALUACION SE PONE UNA OPCION DIFERENTE 
                case 0:
                        estadoEvaluacion = `<button class="btn btn-success rounded-pill px-4 shadow evaluar-btn" id="btnEvaluar-${personal.id_personal}" data-id_personal="${personal.id_personal}">Evaluar</button>`;
                    break;
                case 1:
            ...
            const nombrePersonal = `${personal.nombre_personal} ${personal.apaterno_personal} ${personal.amaterno_personal}`;

            const card360 = document.createElement('div'); // SE CREA UN ELEMENTO HTML QUE ALMACENARA EL CONTENIDO
            ...
            `; 
            lista360.appendChild(card360); // SE EL AGREGA EL ELEMENTO CREADO A EL ELEMENTO HTML ORIGINAL EL EL .html
        });

        const cards = lista360.querySelectorAll('.col');
...
async function cargarPreguntasModal(id_personal) {
    try {
        const res = await fetch(`/getPreguntas360`, { credentials: 'include' }); //OBTENER LAS PREGUNTAS DE LA API
        const data = await res.json();
        ...
        // HEADER - MODAL
        const tituloModal = '360';// AGREGAR TITULO 
        ...
        if (totalPreguntas % 3 == 0 || totalPreguntas % 3 == 2){ // ES 0 PORQUE ESO QUIERE DECIR QUE EL TOTAL DE PREGUNTAS ES MULTIPLO DE 3 PUES NO HAY RESIDUO  Y QUE LA ULTIMA PAGINA ESTA OCUPADA POR 3 PREGUNTAS
            ...
            paginaModalBody.innerHTML = `
                ...
                <textarea class="form-control mb-3 d-none" placeholder="Aspectos a positivos en ${tituloModal.toLowerCase()}: ..." id="comentario360Positivo"></textarea>
                <div class="mb-3 p-3 rounded-4" style="background-color: #eF2d3b">
                ...
                <textarea class="form-control mb-3 d-none" placeholder="Aspectos a negativos en ${tituloModal.toLowerCase()}: ..." id="comentario360Negativo"></textarea>
            ...
            contenidoComentarioPositivo.id = 'comentario360Positivo';
            ...
            contenidoComentarioNegativo.id = 'comentario360Negativo';
        ...
        respuestasComentarioPositivo.forEach(input => { // HACER QUE CADA OPCION (INPUT RADIO) TENGA UN EVENTO PARA SABER CUANDO UNO ESTA SELECCIONADO
            input.addEventListener('change', () => {
                ...
                const comentario360Positivo = document.getElementById('comentario360Positivo');
                // DEPENDIENDO DE LA RESPUESTA APARECE EL ESPPACIO PARA COMENTAR O NO
                if (comentarioPositivoSeleccionValor === "1") {
                    comentario360Positivo.classList.remove('d-none');
                }else{
                    comentario360Positivo.classList.add('d-none');
                }
            });
        });
        ...
        respuestasComentarioNegativo.forEach(input => { // HACER QUE CADA OPCION (INPUT RADIO) TENGA UN EVENTO PARA SABER CUANDO UNO ESTA SELECCIONADO
            input.addEventListener('change', () => {
                ...
                const comentario360Negativo = document.getElementById('comentario360Negativo');
                // DEPENDIENDO DE LA RESPUESTA APARECE EL ESPPACIO PARA COMENTAR O NO
                if (comentarioNegativoSeleccionValor === "1") {
                    comentario360Negativo.classList.remove('d-none');
Par
                    comentario360Negativo.classList.add('d-none');
                }
            });
        });
        ...
        btnTerminarEvaluacion.addEventListener('click', async ()=>{
            ...
            if (comentarioPositivoSeleccionValor == 1 && comentario360Positivo.value == ''){ // QUIERE DECIR QUE SI TIENE COMENTARIO POSITIVO PERO ESTA VACIO POR LO QUE SE ALERTA 
                await Swal.fire({
                    icon: 'error',
            ...
            if (comentarioNegativoSeleccionValor == 1 && comentario360Negativo.value == ''){ // QUIERE DECIR QUE SI TIENE COMENTARIO NEGATIVO PERO ESTA VACIO POR LO QUE SE ALERTA 
                await Swal.fire({
                    icon: 'error',
                    title: 'El apartado para comentar esta vacio'
                });
            ...
            if (comentarioPositivoSeleccionValor == 1 && comentario360Positivo.value != ''){ // QUIERE DECIR QUE SI TIENE COMENTARIO POSITIVO Y QUE NO ESTA VACIO EL COMENTARIO
                comentariosPersonal.push({
                    'tipo_comentario': 1,
                    'comentario_personal': comentario360Positivo.value 
                });
            }

            if (comentarioNegativoSeleccionValor == 1 && comentario360Negativo.value != ''){ // QUIERE DECIR QUE SI TIENE COMENTARIO NEGATIVO Y QUE NO ESTA VACIO EL COMENTARIO
                comentariosPersonal.push({
                    'tipo_comentario': 0,
                    'comentario_personal': comentario360Negativo.value 
                });
            }
            ...
            const resulado = await guardarRespuestas360(id_personal,respuestasPersonal,comentariosPersonal);
            if(resulado.success){
                await Swal.fire({
                    icon: 'success',
...
async function guardarRespuestas360 (id_personal,respuestas,comentarios){
    try {
        ...
        const res = await fetch('/postRespuestas360', {
            method: 'POST',
            headers: {
