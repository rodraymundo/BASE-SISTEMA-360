...
const listaCounselor = document.getElementById('listaCounselor'); // DIV QUE ALMACENARA LA CARD DE COUNSELOR
...
async function cargarCounselor() {
    try {
        const res = await fetch('/getCounselor', { credentials: 'include' }); //OBTENER EL COUNSELOR CON LA API
        const data = await res.json();
        if (!data.success) throw new Error('No se pudo obtener el counselor');

        const counselor = data.counselor[0];
        let estadoEvaluacion = '';
        switch (counselor.estado_evaluacion_counselor) { //SEGUN EL ESTADO DE EVALUACION SE PONE UNA OPCION DIFERENTE 
            case 0:
                    estadoEvaluacion = `<button class="btn btn-success rounded-pill px-4 shadow evaluar-btn" id="btnEvaluar-${counselor.id_personal}" data-id_personal="${counselor.id_personal}">Evaluar</button>`;
                break;
            case 1:
                    estadoEvaluacion = `<p class="fw-bold text-uppercase text-success">Counselor evaluado correctamente</p>`;
                break;            
            default:
                break;
        }

        const nombreCounselor = `${counselor.nombre_personal} ${counselor.apaterno_personal} ${counselor.amaterno_personal}`;

        const cardCounselor = document.createElement('div'); // SE CREA UN ELEMENTO HTML QUE ALMACENARA EL CONTENIDO
        cardCounselor.className = 'col mb-3'; // SE LE AGREGAN LAS CLASES PRINCIPALES A EL ELEMENTO HTML Y ABAJO EL CONTENIDO QUE ALMACENARA
        cardCounselor.innerHTML = `  
            <div class="card h-100 shadow rounded-4 p-4">
                <img src='./assets/img/${counselor.img_personal}' class="card-img-top rounded-top-4" alt="Imagen del Counselor">
                <div class="card-body text-center">
                    <h5 class="fw-bold text-uppercase bg-danger text-white rounded-pill py-2 px-3 d-inline-block shadow-lg">${nombreCounselor}</h5>
                </div>
        ...
        `; 
        listaCounselor.appendChild(cardCounselor); // SE EL AGREGA EL ELEMENTO CREADO A EL ELEMENTO HTML ORIGINAL EL EL .html
    } catch (error) {
        console.error('Error al cargar counselor:', error);
    }
}
...
async function cargarPreguntasModal(id_personal) {
    try {
        const res = await fetch(`/getPreguntasCounselor`, { credentials: 'include' }); //OBTENER LAS PREGUNTAS DE LA API
        const data = await res.json();
        ...
        // HEADER - MODAL
        const tituloModal = 'COUNSELOR';// AGREGAR TITULO 
        ...
        if (totalPreguntas % 3 == 0 || totalPreguntas % 3 == 2){ // ES 0 PORQUE ESO QUIERE DECIR QUE EL TOTAL DE PREGUNTAS ES MULTIPLO DE 3 PUES NO HAY RESIDUO  Y QUE LA ULTIMA PAGINA ESTA OCUPADA POR 3 PREGUNTAS
            ...
            paginaModalBody.innerHTML = `
                ...
                <textarea class="form-control mb-3 d-none" placeholder="Aspectos a positivos en ${tituloModal.toLowerCase()}: ..." id="comentarioCounselorPositivo"></textarea>
                <div class="mb-3 p-3 rounded-4" style="background-color: #eF2d3b">
                ...
                <textarea class="form-control mb-3 d-none" placeholder="Aspectos a negativos en ${tituloModal.toLowerCase()}: ..." id="comentarioCounselorNegativo"></textarea>
            ...
            contenidoComentarioPositivo.id = 'comentarioCounselorPositivo';
            ...
            contenidoComentarioNegativo.id = 'comentarioCounselorNegativo';
        ...
        respuestasComentarioPositivo.forEach(input => { // HACER QUE CADA OPCION (INPUT RADIO) TENGA UN EVENTO PARA SABER CUANDO UNO ESTA SELECCIONADO
            input.addEventListener('change', () => {
                ...
                const comentarioCounselorPositivo = document.getElementById('comentarioCounselorPositivo');
                // DEPENDIENDO DE LA RESPUESTA APARECE EL ESPPACIO PARA COMENTAR O NO
                if (comentarioPositivoSeleccionValor === "1") {
                    comentarioCounselorPositivo.classList.remove('d-none');
                }else{
                    comentarioCounselorPositivo.classList.add('d-none');
                }
            });
        });
        ...
        respuestasComentarioNegativo.forEach(input => { // HACER QUE CADA OPCION (INPUT RADIO) TENGA UN EVENTO PARA SABER CUANDO UNO ESTA SELECCIONADO
            input.addEventListener('change', () => {
                ...
                const comentarioCounselorNegativo = document.getElementById('comentarioCounselorNegativo');
                // DEPENDIENDO DE LA RESPUESTA APARECE EL ESPPACIO PARA COMENTAR O NO
                if (comentarioNegativoSeleccionValor === "1") {
                    comentarioCounselorNegativo.classList.remove('d-none');
                }else{
                    comentarioCounselorNegativo.classList.add('d-none');
                }
            });
        });
        ...
        btnTerminarEvaluacion.addEventListener('click', async ()=>{
            ...
            if (comentarioPositivoSeleccionValor == 1 && comentarioCounselorPositivo.value == ''){ // QUIERE DECIR QUE SI TIENE COMENTARIO POSITIVO PERO ESTA VACIO POR LO QUE SE ALERTA 
                await Swal.fire({
                    icon: 'error',
            ...
            if (comentarioNegativoSeleccionValor == 1 && comentarioCounselorNegativo.value == ''){ // QUIERE DECIR QUE SI TIENE COMENTARIO NEGATIVO PERO ESTA VACIO POR LO QUE SE ALERTA 
                await Swal.fire({
                    icon: 'error',
                    title: 'El apartado para comentar esta vacio'
                });
            ...
            if (comentarioPositivoSeleccionValor == 1 && comentarioCounselorPositivo.value != ''){ // QUIERE DECIR QUE SI TIENE COMENTARIO POSITIVO Y QUE NO ESTA VACIO EL COMENTARIO
                comentariosAlumno.push({
                    'tipo_comentario': 1,
                    'comentario_counselor': comentarioCounselorPositivo.value 
                });
            }

            if (comentarioNegativoSeleccionValor == 1 && comentarioCounselorNegativo.value != ''){ // QUIERE DECIR QUE SI TIENE COMENTARIO NEGATIVO Y QUE NO ESTA VACIO EL COMENTARIO
                comentariosAlumno.push({
                    'tipo_comentario': 0,
                    'comentario_counselor': comentarioCounselorNegativo.value 
                });
            }
            ...
            const resulado = await guardarRespuestasCounselor(id_personal,respuestasAlumno,comentariosAlumno);
            if(resulado.success){
                await Swal.fire({
                    icon: 'success',
...
async function guardarRespuestasCounselor(id_personal,respuestas,comentarios){
    try {
        ...
        const res = await fetch('/postRespuestasCounselor', {
            method: 'POST',
            headers: {
