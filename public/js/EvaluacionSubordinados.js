...
const listaSubordinados = document.getElementById('listaSubordinados'); // DIV QUE ALMACENARA LAS CARD DE SUBORDINADOS 
...
async function cargarSubordinados() {
    try {
        const res = await fetch('/getSubordinados', { credentials: 'include' }); //OBTENER LOS SUBORDINADOS CON LA API
        const data = await res.json();
        if (!data.success) throw new Error('No se pudieron obtener los subordinados');

        data.subordinados.forEach(subordinado => {
            let estadoEvaluacion = '';
            switch (subordinado.estado_evaluacion_subordinado) { //SEFUN EL ESTADO DE EVALUACION SE PONE UNA OPCION DIFERENTE 
                case 0:
                        estadoEvaluacion = `<button class="btn btn-success rounded-pill px-4 shadow evaluar-btn" id="btnEvaluar-${subordinado.id_personal}" data-id_personal="${subordinado.id_personal}">Evaluar</button>`;
                    break;
                case 1:
                        estadoEvaluacion = `<p class="fw-bold text-uppercase text-success">Subordinado evaluado correctamente</p>`;
                    break;            
                default:
                    break;
            }

            const nombreSubordinado = `${subordinado.nombre_personal} ${subordinado.apaterno_personal} ${subordinado.amaterno_personal}`;

            const cardSubordinado = document.createElement('div'); // SE CREA UN ELEMENTO HTML QUE ALMACENARA EL CONTENIDO
            ...
            `; 
            listaSubordinados.appendChild(cardSubordinado); // SE EL AGREGA EL ELEMENTO CREADO A EL ELEMENTO HTML ORIGINAL EL EL .html
        });

        const cards = listaSubordinados.querySelectorAll('.col');
...
async function cargarPreguntasModal(id_personal) {
    try {
        const res = await fetch(`/getPreguntasSubordinado`, { credentials: 'include' }); //OBTENER LAS PREGUNTAS DE LA API
        const data = await res.json();
        ...
        // HEADER - MODAL
        const tituloModal = 'SUBORDINADO';// AGREGAR TITULO 
        ...
        if (totalPreguntas % 3 == 0 || totalPreguntas % 3 == 2){ // ES 0 PORQUE ESO QUIERE DECIR QUE EL TOTAL DE PREGUNTAS ES MULTIPLO DE 3 PUES NO HAY RESIDUO  Y QUE LA ULTIMA PAGINA ESTA OCUPADA POR 3 PREGUNTAS
            ...
            paginaModalBody.innerHTML = `
                ...
                <textarea class="form-control mb-3 d-none" placeholder="Aspectos a positivos en ${tituloModal.toLowerCase()}: ..." id="comentarioSubordinadoPositivo"></textarea>
                <div class="mb-3 p-3 rounded-4" style="background-color: #eF2d3b">
                ...
                <textarea class="form-control mb-3 d-none" placeholder="Aspectos a negativos en ${tituloModal.toLowerCase()}: ..." id="comentarioSubordinadoNegativo"></textarea>
            ...
            contenidoComentarioPositivo.id = 'comentarioSubordinadoPositivo';
            ...
            contenidoComentarioNegativo.id = 'comentarioSubordinadoNegativo';
        ...
        respuestasComentarioPositivo.forEach(input => { // HACER QUE CADA OPCION (INPUT RADIO) TENGA UN EVENTO PARA SABER CUANDO UNO ESTA SELECCIONADO
            input.addEventListener('change', () => {
                ...
                const comentarioSubordinadoPositivo = document.getElementById('comentarioSubordinadoPositivo');
                // DEPENDIENDO DE LA RESPUESTA APARECE EL ESPPACIO PARA COMENTAR O NO
                if (comentarioPositivoSeleccionValor === "1") {
                    comentarioSubordinadoPositivo.classList.remove('d-none');
                }else{
                    comentarioSubordinadoPositivo.classList.add('d-none');
                }
            });
        });
        ...
        respuestasComentarioNegativo.forEach(input => { // HACER QUE CADA OPCION (INPUT RADIO) TENGA UN EVENTO PARA SABER CUANDO UNO ESTA SELECCIONADO
            input.addEventListener('change', () => {
                ...
                const comentarioSubordinadoNegativo = document.getElementById('comentarioSubordinadoNegativo');
                // DEPENDIENDO DE LA RESPUESTA APARECE EL ESPPACIO PARA COMENTAR O NO
...
Par
        ...
        btnTerminarEvaluacion.addEventListener('click', async ()=>{
            ...
            if (comentarioPositivoSeleccionValor == 1 && comentarioSubordinadoPositivo.value == ''){ // QUIERE DECIR QUE SI TIENE COMENTARIO POSITIVO PERO ESTA VACIO POR LO QUE SE ALERTA 
                await Swal.fire({
                    icon: 'error',
            ...
            if (comentarioNegativoSeleccionValor == 1 && comentarioSubordinadoNegativo.value == ''){ // QUIERE DECIR QUE SI TIENE COMENTARIO NEGATIVO PERO ESTA VACIO POR LO QUE SE ALERTA 
                await Swal.fire({
                    icon: 'error',
                    title: 'El apartado para comentar esta vacio'
                });
            ...
            if (comentarioPositivoSeleccionValor == 1 && comentarioSubordinadoPositivo.value != ''){ // QUIERE DECIR QUE SI TIENE COMENTARIO POSITIVO Y QUE NO ESTA VACIO EL COMENTARIO
                comentariosPersonal.push({
                    'tipo_comentario': 1,
                    'comentario_personal': comentarioSubordinadoPositivo.value 
                });
            }

            if (comentarioNegativoSeleccionValor == 1 && comentarioSubordinadoNegativo.value != ''){ // QUIERE DECIR QUE SI TIENE COMENTARIO NEGATIVO Y QUE NO ESTA VACIO EL COMENTARIO
                comentariosPersonal.push({
                    'tipo_comentario': 0,
                    'comentario_personal': comentarioSubordinadoNegativo.value 
                });
            }
            ...
            const resulado = await guardarRespuestasSubordinado(id_personal,respuestasPersonal,comentariosPersonal);
            if(resulado.success){
                await Swal.fire({
                    icon: 'success',
...
async function guardarRespuestasSubordinado (id_personal,respuestas,comentarios){
    try {
        ...
        const res = await fetch('/postRespuestasSubordinado', {
            method: 'POST',
            headers: {
